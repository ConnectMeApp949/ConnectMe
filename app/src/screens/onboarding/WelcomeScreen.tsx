...
export default function WelcomeScreen({ navigation }: Props) {
  ...
  async function handleAppleLogin() {
    ...
    try {
      // Apple Sign In via web browser fallback
      // In production, use expo-apple-authentication for native Apple Sign In
      const redirectUri = 'https://auth.expo.io/@connectme/connectme';
      const clientId = 'com.connectme.app';
      const authUrl = `https://appleid.apple.com/auth/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=name%20email&response_type=code&response_mode=form_post`;
